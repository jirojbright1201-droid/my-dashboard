...
function _lFilter(list){
  const allTs=H.flatMap(h=>h.trades).map(t=>thaiTs(t.date));
...
function renderLogFeeds(){
  ...
  const yF=_lFilter(youAll);
  const yShown=yF.slice(0,logF.lim);
  const footBtn=items=>{
  ...
  const rows=yShown.length
    ?yShown.map(t=>`<div class="lfg-cell you-c">${_lTrade(t)}</div>`).join('')
    :`<div class="lfg-cell you-c"><div class="lf-empty">No items</div></div>`;
...
function expandJr(){logJr=true;renderJrSection('you');}
function collapseJr(){logJr=false;renderJrSection('you');}
...
function renderHoldingsNews(){
  ...
  const filtered=all.filter(n=>thaiTs(n.date)>=cutoff);
  ...
  const cards=shown.length
    ?shown.map(n=>`<div class="hn-card" onclick="openHnNews(${n._i})"><div class="hn-top"><span class="hn-tk">${esc(n.tk)}</span>${moveChip(n)}</div><div class="hn-head">${esc(n.head)}</div><div class="hn-foot"><span class="hn-src">${esc(n.src)}</span><span class="mk-dot">·</span><span class="hn-dt">${esc(n.date)}</span></div></div>`).join('')
...
/* ---------- COMPANY ---------- */
// รวมข้อมูลทุกบริษัทจาก 3 แหล่ง: holdings (ถืออยู่), market.holdings_news, companies (ขายแล้ว/กำลังดู)
function companyRegistry(){
  ...
  Object.values(youTradesByTk).forEach(a=>a.sort((x,y)=>thaiTs(y.date)-thaiTs(x.date)));
  ...
  return {list,newsByTk,youTradesByTk,youSet,soldSet,watchSet};
}
let _coReg=null;
...
// สีไอคอนกลม (avatar) ไล่ตามสถานะถือ: ถืออยู่ > กำลังดู > ขายแล้ว
...
// โดนัทฝั่งคุณ = holdings (ฟ้าไล่เฉด) + เงินสด (เทา)
...
/* ---------- tabs ---------- */
const RENDER={overview:renderOverview,company:renderCompany,log:renderLog,market:renderMarket,thesis:renderThesis};
